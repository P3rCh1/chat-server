...
func Run(cfg *config.Config) {
	...
	r.Group(func(r chi.Router) {
		...
		r.Get("/profile/{userID}", users.AnotherProfile(tools))
		r.Get("/room/{roomID}", rooms.GetRoom(tools))
		r.With(middleware.Throttle(5)).Put("/login", users.Login(tools))
		r.Group(func(r chi.Router) {
			r.Use(mw.Auth(tools.TokenProvider))
			r.Get("/profile", users.MyProfile(tools))
			r.Put("/change-name", users.ChangeName(tools))
			r.Post("/create-room", rooms.Create(tools))
